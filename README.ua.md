# Привіт! Я Михайло 👋

<p align="left">
  <a href="./README.md">🇺🇸 English</a> | <b>🇺🇦 Українська</b>
</p>

Я розробник з досвідом створення Telegram-ботів, веб-застосунків та інтеграцій для e-commerce платформ. Мені подобається будувати чисті, масштабовані рішення та автоматизовувати процеси.

📄 **[Мій інтерактивний сайт-резюме](https://mykhailo-chernykh.netlify.app/)**

---

### 🛠️ Технологічний стек

*   **Мови програмування:** Python, PHP, JavaScript, Java, C#
*   **Веб & E-commerce:** PrestaShop, HTML5, CSS3, REST APIs
*   **Бази даних & Інструменти:** PostgreSQL, SQLite, MySQL, Docker, Git
*   **Бібліотеки & Фреймворки:** Pyrogram, Aiogram, SQLAlchemy

---

### 📂 Обрані проєкти

*   🕵️ **[CodeNames Showcase](https://github.com/chernykh-mykhailo/CodeNames-Showcase)** — Повноцінна реалізація настільної гри «Кодові Імена» у Telegram з кланами, AI-капітаном, тактичними бафами, казино та мультимовністю.
*   🚀 **[EmojiStickerCopyBot](https://github.com/chernykh-mykhailo/EmojiStickerCopyBot)** — Telegram-бот на Python для швидкого копіювання та збереження стікерпаків та емодзі.
*   🤖 **[PrestaShop AI Chatbot](https://github.com/chernykh-mykhailo/prestashop-simplebot-ai)** — Інтеграція штучного інтелекту Gemini безпосередньо у PrestaShop для підтримки клієнтів, обробки FAQ та допомоги покупцям.
*   🎮 **[ClassicPlayBot Showcase](https://github.com/chernykh-mykhailo/ClassicPlayBot-Showcase)** — Модульна ігрова платформа для Telegram з віртуальною економікою, різноманітними ігровими модулями (карткові ігри на кшталт Дурак/Блекджек, настільні та рольові механіки) та мультимовністю.
*   🛍️ **[PrestaShop Extensions](https://github.com/chernykh-mykhailo/prestashop-extensions)** — Колекція кастомних рішень та модулів для платформи PrestaShop.
*   🎮 **[LanPlaySwitch](https://github.com/chernykh-mykhailo/LanPlaySwitch)** — Десктопний інструмент на C# для підключення Nintendo Switch до LAN-мереж.
*   🖱️ **[AutoClicker](https://github.com/chernykh-mykhailo/AutoClicker)** — Десктопна утиліта автоматизації для імітації кліків мишкою та натискання клавіш.

---

### 📊 Моя GitHub Статистика

<p align="left">
  <img src="https://github-stats-extended.vercel.app/api?username=chernykh-mykhailo&show_icons=true&theme=nord&count_private=true&hide_rank=true" alt="Mykhailo's GitHub Stats" height="180px"/>
  <img src="https://github-stats-extended.vercel.app/api/top-langs/?username=chernykh-mykhailo&layout=compact&theme=nord&count_private=true" alt="Top Languages" height="180px"/>
</p>

<p align="left">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=chernykh-mykhailo&theme=nord" alt="GitHub Streak" height="180px"/>
</p>

---

### 📬 Як зв'язатися зі мною
*   **Telegram:** [@Div4unka_z_kare](https://t.me/Div4unka_z_kare)
